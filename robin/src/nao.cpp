...
void Nao::updateLoop() {   
   ...
   while (true) {
      ...
      while (true) {
         ...
         ratio = (currSupport == LEGS ? min(2.5 / maxVelocity, 1.0) : min(1.5 / maxVelocity, 1.0)); 
...
void Nao::setFreeLegAnkleAngles(vector<float>& nextJoints, const vector<float>& curr) {
   ...
   double angle;
   if (currSupport == LLEG) {
      coordinatesFL = motion->getPosition("RFoot/FSR/FrontLeft", 1, false);
      coordinatesRL = motion->getPosition("RFoot/FSR/RearLeft", 1, false);   
      coordinatesRR = motion->getPosition("RFoot/FSR/RearRight", 1, false);
      ...
      coordinatesFL = motion->getPosition("LFoot/FSR/FrontLeft", 1, false);
      coordinatesRL = motion->getPosition("LFoot/FSR/RearLeft", 1, false);  
      coordinatesRR = motion->getPosition("LFoot/FSR/RearRight", 1, false);
