...
{   
   ...
   jointsLowerLim = std::vector<AngleType>(lowerLim, lowerLim + ANGLES);
   jointsUpperLim = std::vector<AngleType>(upperLim, upperLim + ANGLES);
   ...
   //--------RELBOW
   KMatTransf::makeTranslation(T[FR_BASE_T+CHAIN_R_ELBOW], 0.0, -(ShoulderOffsetY), ShoulderOffsetZ);
	KMatTransf::makeRotationXYZ(T[FR_END_T+CHAIN_R_ELBOW], -PI_2, 0.0, -PI_2);
   KMatTransf::makeTranslation(t1, 0.0, 0.0, -UpperArmLength);
	T[FR_END_T+CHAIN_R_ELBOW]*=t1;

   //--------RSHOULDER
   KMatTransf::makeTranslation(T[FR_BASE_T+CHAIN_R_SHOULDER], 0.0, -(ShoulderOffsetY), ShoulderOffsetZ);
	KMatTransf::makeRotationXYZ(T[FR_END_T+CHAIN_R_SHOULDER], -PI_2, 0.0, -PI_2);   
	...
	//KMatTransf::makeRotationXYZ(T[FR_END_T+CHAIN_L_ARM], -PI_2, 0.0, -PI_2);
   //KMatTransf::makeTranslation(t1, -HandOffsetZ, ElbowOffsetY, -UpperArmLength);
   //T[FR_END_T+CHAIN_L_ARM]*=t1;
   T[FR_END_T+CHAIN_L_ARM]=T[FR_END_T+CHAIN_R_ARM];

   //--------LELBOW
   KMatTransf::makeTranslation(T[FR_BASE_T+CHAIN_L_ELBOW], 0.0, ShoulderOffsetY, ShoulderOffsetZ);
	KMatTransf::makeRotationXYZ(T[FR_END_T+CHAIN_L_ELBOW], -PI_2, 0.0, -PI_2);
   KMatTransf::makeTranslation(t1, 0.0, 0.0, -UpperArmLength);
	T[FR_END_T+CHAIN_L_ELBOW]*=t1;

   //--------LSHOULDER
   KMatTransf::makeTranslation(T[FR_BASE_T+CHAIN_L_SHOULDER], 0.0, ShoulderOffsetY, ShoulderOffsetZ);
	T[FR_END_T+CHAIN_L_SHOULDER]=T[FR_END_T+CHAIN_R_SHOULDER];
   ...
   //-----RHIP
	//End and Base effectors
   KMatTransf::makeTranslation(T[FR_BASE_T+CHAIN_R_HIP], 0.0, -HipOffsetY, -HipOffsetZ);
	KMatTransf::makeRotationZYX(T[FR_END_T+CHAIN_R_HIP],  PI ,  -PI_2, 0.0);	
	KMatTransf::makeTranslation(t1, 0.0, 0.0, 0.0);
	T[FR_END_T+CHAIN_R_HIP]*=t1;

   //-----RKNEE
	//End and Base effectors
   KMatTransf::makeTranslation(T[FR_BASE_T+CHAIN_R_KNEE], 0.0, -HipOffsetY, -HipOffsetZ);
	KMatTransf::makeRotationZYX(T[FR_END_T+CHAIN_R_KNEE],  PI ,  -PI_2, 0.0);	
	KMatTransf::makeTranslation(t1, 0.0, 0.0, -ThighLength);
	T[FR_END_T+CHAIN_R_KNEE]*=t1;

   //-----RANKLE
	//End and Base effectors
	KMatTransf::makeTranslation(T[FR_BASE_T+CHAIN_R_ANKLE], 0.0, -HipOffsetY, -HipOffsetZ);
	KMatTransf::makeRotationZYX(T[FR_END_T+CHAIN_R_ANKLE], PI , -PI_2, 0.0);
   KMatTransf::makeTranslation(t1, 0.0, 0.0, -TibiaLength);
   T[FR_END_T+CHAIN_R_ANKLE]*=t1;
   ...
   //-----LHIP
	//End and Base effectors
   KMatTransf::makeTranslation(T[FR_BASE_T+CHAIN_L_HIP], 0.0, HipOffsetY, -HipOffsetZ);
	KMatTransf::makeRotationZYX(T[FR_END_T+CHAIN_L_HIP], PI , -PI_2, 0.0);	
	KMatTransf::makeTranslation(t1, 0.0, 0.0, 0.0);
	T[FR_END_T+CHAIN_L_HIP]*=t1;

   //------LKNEE
	//End and Base effectors
	KMatTransf::makeTranslation(T[FR_BASE_T+CHAIN_L_KNEE], 0.0, HipOffsetY, -HipOffsetZ);
	TBaseLLegInv=T[FR_BASE_T+CHAIN_L_KNEE];	
	KMatTransf::makeRotationZYX(T[FR_END_T+CHAIN_L_KNEE], PI , -PI_2, 0.0);	
	KMatTransf::makeTranslation(t1, 0.0, 0.0, -ThighLength);
	T[FR_END_T+CHAIN_L_KNEE]*=t1;

   //-----LANKLE
	//End and Base effectors
	KMatTransf::makeTranslation(T[FR_BASE_T+CHAIN_L_ANKLE], 0.0, HipOffsetY, -HipOffsetZ);
	KMatTransf::makeRotationZYX(T[FR_END_T+CHAIN_L_ANKLE], PI, -PI_2, 0.0);	
   KMatTransf::makeTranslation(t1, 0.0, 0.0, -TibiaLength);
   T[FR_END_T+CHAIN_L_ANKLE]*=t1;
	//------LLEG
	//End and Base effectors
...
double NAOKinematics::getJoint(const int jointID) {
   return joints[jointID];
}//getJoint

void NAOKinematics::changeJoint(const int jointID, const double amount) 
{
   joints[jointID] += amount;
   joints[jointID] = std::min(joints[jointID], jointsUpperLim[jointID]);
   joints[jointID] = std::max(joints[jointID], jointsLowerLim[jointID]);
   prepareForward();
}

void NAOKinematics::setJoint(const int jointID, const double amount) 
{
   joints[jointID] = amount;
   joints[jointID] = std::min(joints[jointID], jointsUpperLim[jointID]);
   joints[jointID] = std::max(joints[jointID], jointsLowerLim[jointID]);   
	...
	switch(ch)
	{
      ...
      case CHAIN_R_KNEE:
		...
		case CHAIN_R_LEG:
				...
				chsize=LEG_SIZE;
				break;
      case CHAIN_L_SHOULDER:
            frstart=(Frames)L_ARM;
				chsize=SHOULDER_SIZE;
				break;
      case CHAIN_L_ELBOW:            
            frstart=(Frames)L_ARM;
				chsize=ELBOW_SIZE;
...
void NAOKinematics::prepareForward(KDeviceLists::ChainsNames ch)
{
	if(ch==CHAIN_L_ARM || ch==CHAINS_SIZE)
	{
		...
		/*Correct one BUT THE FIX does not change anything*/
