 ...
 * \file NAOKinematics.h
 ...
 There are several fixes to transform the "real" chain to the "fake" one and they are located at the NAOKinematics.cpp file and at the KinematicsDefines header.
...
private:
   ...
   std::vector<AngleType> jointsLowerLim;
   std::vector<AngleType> jointsUpperLim;
...
public:
	 ...
	 * */
   void changeJoint(const int jointID, const double amount);
   void setJoint(const int jointID, const double amount);
   double getJoint(const int jointID);
